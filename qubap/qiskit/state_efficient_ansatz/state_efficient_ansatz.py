...
def ansatz_constructor(
    ...
    qc.compose(
        U_1, qubits=range(int(n_qbits / 2)), clbits=None, inplace=True
    )  # Compose entangled layer cnots
    if set_barrier == True:
        qc.barrier()

    qc.compose(
        ent_l, qubits=range(n_qbits), clbits=None, inplace=True
    )  # Compose entangled layer cnots
    if set_barrier == True:
        qc.barrier()

    qc.compose(
        U_2, qubits=range(int(n_qbits / 2)), clbits=None, inplace=True
    )  # Compose entangled layer cnots
    qc.compose(
        U_3, qubits=range(int(n_qbits / 2), n_qbits), clbits=None, inplace=True
    )  # Compose entangled layer cnots
